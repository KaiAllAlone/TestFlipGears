...
class FreshnessDetectionModel():
    def __init__(self):
        ...
        self.class_indices = [list(self.model.names.keys())[list(self.model.names.values()).index(cls)] 
    ...
    def process_frame(self, frame, prev_frame=None, write=True):
        ...
        if ssim <= 0.9:
            results = self.model(frame, stream=True, max_det=1,classes=self.class_indices)  # Perform inference
            for r in results:
                for box in r.boxes:
                    ...
                    if(detection_time-self.latest_detection_time>20 or self.prev_class_name!=class_name):
                        if write:
                        ...
                        self.remaining_days.append(self.calculate_remaining_days(confidence*10))
